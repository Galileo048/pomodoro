...
class SecantToTangentScene(Scene):
    def construct(self):
        ...
        # Info panel
        panel = Rectangle(width=3.8, height=2.6, fill_color=BLACK,
                          fill_opacity=0.8, stroke_color=GRAY, stroke_width=1)
        panel.to_corner(UR, buff=0.25)
        self.add(panel)
        pc = panel.get_center()

        dy_lbl = Text("Dy =", font_size=24).move_to(pc + UP * 0.85 + LEFT * 1.2)
        dy_num = always_redraw(lambda: Text(
            f"{f(a + h_tracker.get_value()) - f(a):.4f}",
            font_size=24, color=YELLOW).next_to(dy_lbl, RIGHT, buff=0.15))

        dx_lbl = Text("Dx =", font_size=24).move_to(pc + UP * 0.25 + LEFT * 1.2)
        dx_num = always_redraw(lambda: Text(
            f"{h_tracker.get_value():.4f}",
            font_size=24, color=YELLOW).next_to(dx_lbl, RIGHT, buff=0.15))

        slope_lbl = Text("Dy/Dx =", font_size=24).move_to(pc + DOWN * 0.35 + LEFT * 1.2)
        ...
        self.add(dy_lbl, dy_num, dx_lbl, dx_num, slope_lbl, slope_num, lim_lbl)
        ...
        t_lbl = Text(f"切线 y={f_prime:.2f}x+{f(a)-f_prime*a:.2f}",
        ...
        self.play(Create(SurroundingRectangle(
            VGroup(slope_lbl, slope_num), color=GREEN, buff=0.1)))
        self.wait(3)
